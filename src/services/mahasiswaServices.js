...
const entryDataIrs = async (data) => {
  try {
    ...
    if (!['Aktif', 'Cuti', 'Mangkir', 'Lulus'].includes(statusMhs.statusAktif))
      throw new Error('Status mahasiswa tidak valid');
...
const getProfileMahasiswa = async (data) => {
  try {
    ...
    const profile = {
      namaKab: result.namaKab,
      namaProv: result.namaProv,
      foto: result.foto,
      kodeKab: result.kodeKab,
      kodeProv: result.kodeProv,
      nama: result.nama,
      nim: result.nim,
      angkatan: result.angkatan,
